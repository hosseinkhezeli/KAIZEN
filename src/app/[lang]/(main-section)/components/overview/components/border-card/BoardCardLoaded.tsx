...
const BoardCardLoaded: FC<BoardCardLoadedProps> = ({ boardInfo, onClick }) => {
    return (
        <Stack onClick={onClick} sx={styles.card}>
            <Box sx={styles.header}>
                <DocumentIcon width={16} height={16} />
                <Typography variant='subtitle2'>{boardInfo?.title}</Typography>
            </Box>
            ...
            <Box sx={styles.labels}>
                {boardInfo?.labels?.map((label, idx) => (
                    <Typography key={idx} variant='caption' sx={styles.label}>
                        {label}
                    </Typography>
                ))}
            </Box>
            <Box className={'board-card-bg'} sx={styles.imageContainer}>
                <Image
...
export default BoardCardLoaded;
